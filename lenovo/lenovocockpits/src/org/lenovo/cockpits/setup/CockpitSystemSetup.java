 ...
 *  
 */
package org.lenovo.cockpits.setup;
...
import org.lenovo.cockpits.constants.LenovoCockpitsConstants;
 ...
 */
@SystemSetup(extension = LenovoCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(LenovoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(LenovoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						LenovoCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/lenovocockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
